...
const CartProductItem = ({product, handleAddProduct, handleDescreaseProduct, handleDeleteProduct}) => {
    ...
    return (
            ...
            <div className={"cartProduct"}>
                <div className={"cartProductDelete"}>
                    <button className={"cartProductDeleteButton"} onClick={() => handleDeleteProduct(product)}>
                ...
                <div className={"cartProductItem"}>
                    <img src={process.env.PUBLIC_URL + product.img} className={"cartProductImage"} alt={product.title}></img>
                    <div className={"cartItemInfo"}>
                        <a className={"cartItemInfoTitle"}>{product.title}</a>
                        <a className={"cartItemInfoPrice"}>{product.price} &#8381;</a>
                    </div>
                </div>
                <div className={"cartItemButtonsSum"}>
                    <div className={"cartItemButtons"}>
                        <button className={"descreaseButton"} onClick={()=>handleDescreaseProduct(product)}><div className={"minus"}></div></button>
                        <span className={"cartProductItemAmount"}>{product.amount}</span>
                        <button className={"increaseButton"} onClick={()=>handleAddProduct(product)}><div className={"minusplus"}></div><div className={"plus"}></div></button>
                    </div>
                    <span className={"cartProductItemSum"}>{itemsSum()} &#8381;</span>
                </div>
            </div>
